...
import Geb.Internal  -- forbidden: upstream-eligible files import only Mathlib.* or Geb.Mathlib.*
EOF
bash scripts/lint-imports.sh
# Expected: exit 1, message about forbidden import.
echo "Exit code: $?"
```
...
EOF
bash scripts/lint-imports.sh
# Expected: exit 1, message about prefix leakage.
```
